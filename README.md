...
# Goethe University trading algorithm (A little Roadmap)
The newest approach of a trading algorithm was inspired by the BlackRock algorithm.
Because of this algorithm, Laurence Fink became the most powerful man in the whole USA. The algorithm isn't public, self speaking but we made it our goal to develop our own version.

In the test phase our algorithm made a steady 25% profit, each year, while trading Cryptocurrencies for four Years now.
So now we know that its time.
Rougly speaking Goethe Universities algorithm takes news, hints and calls from all over the world and converts it to useable data. A team of 7 professors worked on this project for over 7 years now.
...
# Goals:
  ...
  - Distribute all of the 10.000.000 Hiroas
  
  
  
# If we cant reach our Investment goal of 2.000.000 $
  - All the funds will be returned back to the owners
  - The Goethe University trading algorithm will be made public and is for everyone to use
  - As the algorithm requieres a lot of calculating power, investors can invest their Hiroas directly with a 0.5% fee.
  - The Payout will be bi-weekly
...
You may purchase the Hiroas only after getting in contact with us via coin@hiroas.xyz
Any donations made prior can benefit your position in the investor pool.


During the ICO, 100% of the tokens will be distributed, that is, 10,000,000 of Hiroas Tokens (Total Supply). The number of the Hiroas Tokens allowed for purchase by one user is unlimited at fixed price of $0,20 per Hiroas Token. The Hiroas Tokens will be issued only once. The Hiroas Tokens will be Ethereum-based cryptographic tokens of value.

# So, Hiroas...

Hiroas will choose the assets (a mixture of bitcoin, ethereum, altcoins, tokens and other cryptoassets) based on our complex trading models and strategies that we made thanks to the use of Deep Learning techniques combined with our financial expertise.

Before being available, there will be a beta available and a training period and model test for Hiroas where all results will be publicly accessible. Thanks to the technology behind Hiroas and the trading models developed, she will be able to choose her investment approach and vary the strategies in a self-determined way (monitored by our team at all times) according to the conditions and moments of the market. The portfolio can be monitored by any investor thanks to our monitoring and auditing tools. In addition, external bimonthly auditions will be conducted to provide the necessary transparency to investors.
...
There are some extraordinary cases where a software bug, Internet congestion, or a malicious block producer will create two or more forks. For absolute certainty that a transaction is irreversible, a node may choose to wait for confirmation by 15 out of the 21 block producers. Based on a typical configuration of the Hiroas software, this will take an average of 45 seconds under normal circumstances. By default all nodes will consider a block confirmed by 15 of 21 producers irreversible and will not switch to a fork that excludes such a block regardless of length.
...
The Hiroas software requires every transaction to include the hash of a recent block header. This hash serves two purposes:
...
Using the Hiroas software, accounts can define named permission levels each of which can be derived from higher level named permissions. Each named permission level defines an authority; an authority is a threshold multi-signature check consisting of keys and/or named permission levels of other accounts. For example, an account's "Friend" permission level can be set for the account to be controlled equally by any of the account's friends.

Another example is the Steem blockchain which has three hard-coded named permission levels: owner, active, and posting. The posting permission can only perform social actions such as voting and posting, while the active permission can do everything except change the owner. The owner permission is meant for cold storage and is able to do everything. The Hiroas software generalizes this concept by allowing each account holder to define their own hierarchy as well as the grouping of actions.

# Named Message Handler Groups

The Hiroas software allows each account to organize its own message handlers into named and nested groups. These named message handler groups can be referenced by other accounts when they configure their permission levels.
...
The Hiroas technology also allows all accounts to have an "owner" group which can do everything, and an "active" group which can do everything except change the owner group. All other permission groups are derived from "active".
...
Time is a critical component of security. In most cases, it is not possible to know if a private key has been stolen until it has been used. Time based security is even more critical when people have applications that require keys be kept on computers connected to the internet for daily use. The Hiroas software enables application developers to indicate that certain messages must wait a minimum period of time after being included in a block before they can be applied. During this time they can be cancelled.
...
The Hiroas software provides users a way to restore control of their account when their keys are stolen. An account owner can use any owner key that was active in the last 30 days along with approval from their designated account recovery partner to reset the owner key on their account. The account recovery partner cannot reset control of the account without the help of the owner.
...
When using merkle proofs from outside blockchains, there is a significant difference between knowing that all transactions processed are valid and knowing that no transactions have been skipped or omitted. While it is impossible to prove that all of the most recent transactions are known, it is possible to prove that there have been no gaps in the transaction history. The Hiroas software facilitates this by assigning a sequence number to every message delivered to every account. A user can use these sequence numbers to prove that all messages intended for a particular account have been processed and that they were processed in order.
